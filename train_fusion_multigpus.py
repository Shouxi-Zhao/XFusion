...
from net11 import MultiFocusTransNet
# from net8_fustormer import Fustormer as MultiFocusTransNet
# from multifocus_dataset import MultifocusGrayDataset
from dataset_coco import CocoMultifocus, tensor2cvimage, cvimage2tensor,tensor_normalize, tensor_unnormalize, likely_resize_640_480_, train_file_list, valid_file_list, ImageTools

# device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
# device = torch.device("cpu")

# GPU_DEVICES = [0,1,2,3]
GPU_DEVICES = [0,1,2,4,5,7]
...
def test_RGBTogether(epoch, model, dataloader):
    ...
    with torch.no_grad():
        for src, b0, b1, b2 in test_bar:
            ...
            # input = torch.cat([b0,b1,b2], dim = 1)
            # input = input.cuda(device=GPU_DEVICES[0])
                
            # ypred = model(input)
            ypred = model(b0.cuda(device=GPU_DEVICES[0]), b2.cuda(device=GPU_DEVICES[0]) ) #, b2.cuda(device=GPU_DEVICES[0]))

            num += b
            for bi in range(b):
                s = tensor2cvimage(tensor_unnormalize(src[bi,:,:,:]))
                y = tensor2cvimage(tensor_unnormalize(ypred[bi,:,:,:]))

                ps = psnr(s, y, data_range=255)
...
def test_Grayonly(epoch, model, dataloader):
    global BEST_PSNR_SSIM
    avg_psnr = 0
    avg_ssim = 0
    num = 0
    ...
    with torch.no_grad():
        for src, b0, b1, b2 in test_bar:
            ...
            num += b
            for bi in range(b):
                s = tensor2cvimage(tensor_unnormalize(src[bi,0:1,:,:])).reshape(hi, wi )
                y = tensor2cvimage(tensor_unnormalize(ypred[bi,0:1,:,:])).reshape(hi, wi )

                ps = psnr(s, y, data_range=255)
                sm = ssim(y, s, data_range=255, multichannel=False)
                # print('psnr {}.. ssim {}.. '.format(ps, sm))
                avg_psnr += ps
                avg_ssim += sm
                test_bar.set_description('psnr {:.5f}.. ssim {:.5f}.. '.format(ps, sm))
...
def test_yonly(epoch, model, dataloader):
    global BEST_PSNR_SSIM
    avg_psnr = 0
    avg_ssim = 0
    num = 0
    ...
    with torch.no_grad():
        for src, b0, b1, b2 in test_bar:
            ...
            num += b
            for bi in range(b):
                s = tensor2cvimage(tensor_unnormalize(src[bi][0])).reshape(hi, wi, channel)
                s = ImageTools.convert_ycbcr_to_rgb(s)

                fuse_cb = ImageTools.ycbcr_merge_c(b0[bi,1,:,:], b1[bi,1,:,:], b2[bi,1,:,:])
                fuse_cr = ImageTools.ycbcr_merge_c(b0[bi,2,:,:], b1[bi,2,:,:], b2[bi,2,:,:])
                result = np.array([ypred[bi,0,:,:], fuse_cb, fuse_cr])

                result = result.transpose(1,2,0)
                result = ImageTools.convert_ycbcr_to_rgb(result)
...
def train(epoch, model, dataloader, criterion, optimizer, writer):
    ...
    for src, b0, b1, b2 in train_bar:
        ...
        #------------------ train for RGB together ----------------------
        loss = 0
        ssim_loss = 0
        train_label = src.cuda(device=GPU_DEVICES[0])
        # input = torch.cat((b0, b1, b2), dim = 1) # B 9 H W
        # input = input.cuda(device=GPU_DEVICES[0])

        # y = model(input) # B 3 H W
        y = model(b0.cuda(device=GPU_DEVICES[0]), b2.cuda(device=GPU_DEVICES[0])) #, b2.cuda(device=GPU_DEVICES[0]))
        loss += criterion(y, train_label)
        ssim_loss += ssim_lossfun( y, train_label, data_range=1, size_average=True)
        #------------------ end -- train for RGB together ----------------------
        ...
        # input = torch.cat((b0[:,0:1,:,:], b1[:,0:1,:,:], b2[:,0:1,:,:]), dim = 1)
        # input = input.cuda(device=GPU_DEVICES[0])
            
        # yi = model(input)

        # loss += criterion(yi, train_label[:,0,:,:])
        # ssim_loss += ssim_lossfun( yi, train_label[:,0:1,:,:], data_range=1, size_average=True)
        #------------------ end -- train for Y only ----------------------
        ...
        # loss = criterion(ypred, train_label)
        # ssim_loss = ssim_lossfun( ypred, train_label, data_range=1, size_average=True) #
        #------------------ end -- train for RGB apart ----------------------

        loss_total = loss + (1-ssim_loss)* 2
        loss_total.backward()
        optimizer.step()
...
def train_task_multi_fusion():
    ...
    # train_dataset = CocoMultifocus(train_file_list, half_size = True, mode = "YCbCr")
    # test_dataset = CocoMultifocus(valid_file_list, half_size = True, mode = "YCbCr")
    train_dataset = CocoMultifocus(train_file_list, re_size = 0.75, mode = "RGB")
    test_dataset = CocoMultifocus(valid_file_list, re_size = 1.0, mode = "RGB")
    ...
    model = MultiFocusTransNet(32, 2) #, dim=24, num_blocks=[4,6,6,8], dual_pixel_task=True)
    ...
    checkpoint = torch.load('ckpts/fusion_385_PS_SM(26.769_0.842).model', map_location=torch.device('cpu'))
    model.load_state_dict(checkpoint)
    #model.load_state_dict(torch.load('ckpts/fusion_{}.model'.format(epoch_start)), map_location= lambda storage, loc : storage.cuda(0))#{'cuda:1':'cuda:0'})

    criterion = torch.nn.L1Loss()
    # criterion = torch.nn.MSELoss()
    optimizer1 = optim.SGD(model.parameters(), lr=0.001, weight_decay=1e-8, momentum=0.9)
    # optimizer = optim.Adam(model.parameters(), lr=0.000001, weight_decay=1e-8)

    optimizer2 = optim.SGD(model.parameters(), lr=0.0001, weight_decay=1e-8, momentum=0.9)
    
    optimizer3 = optim.SGD(model.parameters(), lr=0.00001, weight_decay=1e-8, momentum=0.9)

    for epoch in range(EPOCH):
        if epoch < epoch_start:
        #     # epoch += 1
            continue
        optimizer = optimizer1 if epoch <= 200 else optimizer2
        optimizer = optimizer3 if epoch >=300 else optimizer
        train(epoch, model, train_loader, criterion, optimizer, writer)
        test_RGBTogether(epoch, model, test_loader) #, criterion, writer)
        # if epoch % 20 == 0:
